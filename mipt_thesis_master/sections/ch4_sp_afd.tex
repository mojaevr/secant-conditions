...
(Armijo line search до входа в окрестность, где~\eqref{eq:wp-condition}
выполнено).
...
$L_1/2$, достаточно сменить дефолтное $\tau = 1/4$ на $\tau \le 1/16$
...
В дальнейшем мы используем $\tau = 1/4$ как дефолт; асимптотика теоремы
...
роль глобализации шага и аналогична line search Армихо для случая
...
\emph{first-passage time} $T(\varepsilon) := \min\{k:\,\min_{j\le k}\Norm{F(x_j)}\le\varepsilon\}$
и линейный фит $\log T(\varepsilon) = a\cdot\log(1/\varepsilon) + \mathrm{const}$
...
SP-AFD). Справа: first-passage time $T(\varepsilon)$ как функция $-\log_{10}\varepsilon$
(медиана по $8$ seeds, IQR~--- бары); линейный фит slope $a$ в
...
(см.~\S\ref{lr:anderson} и работы~\cite{anderson1965,walker2011,fang2009}).
...
Структурная причина~--- плотный мультисекантный fit на смешанной
