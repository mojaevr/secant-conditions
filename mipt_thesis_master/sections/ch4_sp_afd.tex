...
\section{Ключевая лемма: выравнивание $d_{k+1}$ с секантой $s_k$}
\label{sec:viji:key-lemma}

Любое секант-сохраняющее обновление якобиана~--- в т.ч.\ классический
Broyden и SP-Broyden~--- удовлетворяет
$\Norm{(B_{k+1} - J_k) s_k} \le \tfrac{L_1}{2}\Norm{s_k}^2$, где
$J_k = \nabla F(v_k)$; это стандартная оценка секантного остатка
(\cite[Lemma~4.1.12]{dennis1996}, ср.\ \cite{dennis1977}). Нетривиальная
часть в схеме VIJI-Restarted~--- что условие~\eqref{eq:cond14} проверяется
на \emph{следующем} шаге $d_{k+1} = x_{k+1} - v_{k+1}$, а не на секанте
$s_k$, по которой было выполнено обновление. Ключевой вопрос: насколько
$d_{k+1}$ близок к $s_k$?
...
Полное доказательство леммы~\ref{lem:alignment} приведено в
приложении~\ref{app:alignment} (теорема~\ref{thm:app-aln-l1:main})
\emph{условно} относительно одного дополнительного условия~---
асимптотического выравнивания направлений ошибки в окне
\textup{(}\ref{as:dir-align}, аналог классического Powell-выравнивания
секущих в BFGS\textup{)}; стандартная Q-линейная сходимость
$\Norm{e_k}\le\rho_v\Norm{e_{k-1}}$, $\rho_v<1$, используется как
фоновое свойство VIJI-итерации \textup{(}\cite[Theorem~3.4]{Agafonov2024VIJI};
ср.\,\cite[Definition~9.1.5]{dennis1996}\textup{)}. Это даёт константы
$C_a, C_b$, зависящие только от $\mu, L_1, \eta, M_0, c_d, \omega_h, \rho_v, p$.
Квадратичный случай $L_1=0$ получается прямой подстановкой
\textup{(}предложение~\ref{prop:app-aln-l1:dm-quad}: $\Norm{E_k s_k}/\Norm{s_k}\to 0$
безусловно, $C_a=C_b=0$\textup{)}. Безусловный вариант леммы
(без~\ref{as:dir-align}) остаётся гипотезой~\ref{conj:alignment}.
\end{remark}

\section{Условная оптимальная теорема}
\label{sec:viji:theorem}

Комбинируя стандартную оценку секантного остатка
$\Norm{(B_{k+1} - J_k) s_k} \le \tfrac{L_1}{2}\Norm{s_k}^2$
(\cite[Lemma~4.1.12]{dennis1996}) и лемму~\ref{lem:alignment}, получаем
...
$M_0^2 \le \Fro{B_0 - \Jstar}^2 + \widetilde C_{\mathrm{SP}}/((1 - \rho^2)(1 - \bar\rho^2))\,D_0^2$
...
&\quad{} + \Norm{J_k - J_{k+1}}_{op} \Norm{d_{k+1}}.
\end{align*}
Оценим каждое слагаемое:
\begin{itemize}
\item $|\alpha_k| \le \Norm{d_{k+1}}/\Norm{s_k}$ и $\Norm{(B_{k+1} - J_k) s_k} \le (L_1/2)\Norm{s_k}^2$
      по стандартной оценке секантного остатка~\cite[Lemma~4.1.12]{dennis1996},
      поэтому первое слагаемое $\le (L_1/2)\Norm{s_k}\Norm{d_{k+1}}$.
...
\end{equation}
где $C_1 := \tfrac{L_1 C_d}{2} + M_0 C_a + L_1 C_d$.
...
\qquad
K := \tfrac{\widetilde C_{\mathrm{SP}}}{1 - \bar\rho^2},
...
$\Fro{B_0^{(s)} - \Jstar}^2 \le \tilde L_0^2$, где
\begin{equation}
\tilde L_0^2 :=
\begin{cases}
\Fro{I - \Jstar}^2, & B_0^{(s)} = I \text{ (холодный рестарт)}, \\
\Fro{B_0^{(0)} - \Jstar}^2 + \dfrac{\widetilde C_{\mathrm{SP}}\, D_0^2}{(1-\rho^2)(1-\bar\rho^2)},
...
$\Fro{B_k^{(s)} - \Jstar}^2 \le \Fro{B_0^{(s)} - \Jstar}^2 + (\widetilde C_{\mathrm{SP}}/(1-\bar\rho^2))\,\Norm{z^{(s)}-x^{*}}^2$.
...
тем самым $K = \widetilde C_{\mathrm{SP}}/(1-\bar\rho^2)$ (и $\underline\rho$ для
...
выполнения леммы~\ref{lem:alignment}~--- доказанной в
приложении~\ref{app:alignment} (теорема~\ref{thm:app-aln-l1:main}) условно
при единственном дополнительном предположении~\ref{as:dir-align},
безусловный вывод которого зафиксирован как гипотеза~\ref{conj:alignment},~---
...
её утверждение опирается на лемму~\ref{lem:alignment}. Лемма доказана в
приложении~\ref{app:alignment} (теорема~\ref{thm:app-aln-l1:main}) условно
при единственном дополнительном предположении~--- асимптотическом
выравнивании направлений ошибки~\ref{as:dir-align}; стандартная
Q-линейная сходимость~\cite[Theorem~3.4]{Agafonov2024VIJI} используется
как фоновое свойство. Безусловный вывод~\ref{as:dir-align}
зафиксирован как гипотеза~\ref{conj:alignment}. Альтернативно,
безусловный аналог условия~\eqref{eq:cond14} в общем случае
...
приложения~\ref{app:sp-afd-proof}: лемму~\ref{lem:app-afd:dirbound}
...
\item На рассматриваемой задаче SP-Broyden сходится без FD-подкрепления,
      ...
      условие~\ref{as:dir-align} (а с ним и
      гипотеза~\ref{conj:alignment}) выполнено эмпирически.
...
Лемма~\ref{lem:alignment} доказана \emph{условно} в
приложении~\ref{app:alignment} (теорема~\ref{thm:app-aln-l1:main})
при единственном дополнительном предположении~--- асимптотическом
выравнивании направлений ошибки~\ref{as:dir-align}
(аналог Powell-выравнивания секущих в BFGS), дающем константы
$C_a, C_b$ через $\mu, L_1, \eta, M_0, c_d, \omega_h, \rho_v, p$.
Стандартная Q-линейная сходимость VIJI \textup{(}\cite[Theorem~3.4]{Agafonov2024VIJI}\textup{)}
используется как фоновое свойство. Безусловный вывод~\ref{as:dir-align}
из~\ref{as:sm} и теоремы~\ref{thm:bd}~--- открытый теоретический вопрос,
зафиксированный гипотезой~\ref{conj:alignment} и поддержанный численно.
...
\begin{itemize}
\item \emph{Безусловный вывод~\ref{as:dir-align}}: вывод асимптотического
      выравнивания направлений ошибки $\Norm{\hat e_{k-j} - \hat u_k}
      = O(\bar e_k)$ из~\ref{as:sm}, Q-линейной сходимости
      \textup{(}\cite[Theorem~3.4]{Agafonov2024VIJI}\textup{)}
      и bounded deterioration~\ref{thm:bd}~--- замыкающий условную
      теорему~\ref{thm:app-aln-l1:main} и снимающий зависимость
      леммы~\ref{lem:alignment} от дополнительного предположения~---
      предмет отдельной публикации по степенной итерации с медленно
      меняющейся матрицей $I - T_k$.
...
\begin{conjecture}[Асимптотическое выравнивание направлений ошибки в VIJI]
\label{conj:alignment}
В предположениях~\ref{as:sm} и стандартной Q-линейной сходимости
\textup{(}\cite[Theorem~3.4]{Agafonov2024VIJI}\textup{)} для
VIJI-итераций SP-Broyden с $\eta = 10$ при простом по модулю наибольшем
собственном значении $\Jstar$ структурное условие~\ref{as:dir-align}
выполнено с константой $\omega_h$, зависящей только от
$\mu, L_1, \rho_v$ и $M_0$. Эквивалентно: лемма~\ref{lem:alignment}
безусловно справедлива в общем $L_1$-гладком случае с константами
$C_a, C_b$ из теоремы~\ref{thm:app-aln-l1:main}, зависящими только от
$\mu, L_1, \eta, M_0, \rho_v, p$.
\end{conjecture}
