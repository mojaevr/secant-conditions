...
В настоящем тезисе мы доказываем лемму~\ref{lem:alignment} в двух режимах.
В \emph{квадратичном} случае ($F$ линеен по $x-x^{*}$, $L_1=0$) полное
доказательство приведено в приложении~\ref{app:alignment} через
коммутативную алгебру $\mathbb{R}[A]$ и спектральную оценку Бхатии. В
общем $L_1$-гладком случае доказательство приведено в
приложении~\ref{app:alignment-l1} \emph{условно} относительно
дополнительного количественного предположения Дэнниса--Море
(см.~\ref{as:dm-rate} и обсуждение~\ref{rem:dm-rate-status}); это даёт
явные константы $C_a, C_b$, зависящие только от $\mu, L_1, \eta, M_0,
c_d, \zeta_{\mathrm{DM}}$. Безусловный вариант леммы (без~\ref{as:dm-rate})
остаётся гипотезой~\ref{conj:alignment} и поддержан численными
экспериментами раздела~\ref{sec:viji:numerics}.
\end{remark}
...
выполнения леммы~\ref{lem:alignment}~--- безусловно доказанной для
квадратичного случая (приложение~\ref{app:alignment}) и условно для
общего $L_1$-гладкого случая
(приложение~\ref{app:alignment-l1}, теорема~\ref{thm:app-aln-l1:main},
при дополнительном предположении~\ref{as:dm-rate}; безусловный
вывод~\ref{as:dm-rate} зафиксирован как гипотеза~\ref{conj:alignment}),~---
...
её утверждение опирается на лемму~\ref{lem:alignment}. Для квадратичных
$F$ лемма доказана безусловно (приложение~\ref{app:alignment}); для
общего $L_1$-гладкого случая~--- условно при дополнительном
предположении Дэнниса--Море~\ref{as:dm-rate}
(приложение~\ref{app:alignment-l1}, теорема~\ref{thm:app-aln-l1:main}).
Безусловный вывод~\ref{as:dm-rate} из~\ref{as:sm} (Powell-стиль)
...
\item На рассматриваемой задаче SP-Broyden сходится без FD-подкрепления,
      ...
      предположение~\ref{as:dm-rate} (а с ним и
...
Лемма~\ref{lem:alignment} безусловно доказана для квадратичных задач
(приложение~\ref{app:alignment}) через симметричное полиномиальное
исчисление от $\Jstar$ и спектральную оценку Бхатии. В общем
$L_1$-гладком случае она доказана \emph{условно} в
приложении~\ref{app:alignment-l1} (теорема~\ref{thm:app-aln-l1:main})
при дополнительном количественном предположении
Дэнниса--Море~\ref{as:dm-rate}, дающем явные константы
$C_a, C_b$ через $\mu, L_1, \eta, M_0, c_d, \zeta_{\mathrm{DM}}$.
Безусловный вывод~\ref{as:dm-rate} из bounded deterioration
SP-Broyden и Q-линейной сходимости VIJI (Powell-стиль)~--- открытый
теоретический вопрос, зафиксированный гипотезой~\ref{conj:alignment}
и поддержанный численно.
...
\begin{itemize}
\item \emph{Безусловный вывод~\ref{as:dm-rate}}: количественный
      Powell-style аргумент, замыкающий условную
      теорему~\ref{thm:app-aln-l1:main} и снимающий зависимость
      леммы~\ref{lem:alignment} от дополнительной гипотезы~---
      предмет отдельной публикации по геометрии VIJI-итераций.
...
\begin{conjecture}[Безусловный Дэнниса--Море-режим SP-Broyden в VIJI]
\label{conj:alignment}
В предположениях~\ref{as:sm} для VIJI-итераций SP-Broyden с
$\eta = 10$ количественное условие Дэнниса--Море~\ref{as:dm-rate}
выполнено с константой $\zeta_{\mathrm{DM}}$, зависящей только от
$\mu, L_1$ и $M_0$. Эквивалентно: лемма~\ref{lem:alignment}
безусловно справедлива в общем $L_1$-гладком случае с константами
$C_a, C_b$ из теоремы~\ref{thm:app-aln-l1:main}, зависящими только
от $\mu, L_1, \eta, M_0$.
\end{conjecture}
