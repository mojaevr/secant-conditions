...
$\mathtt{maxit}=600$); обе оси~--- в~относительных единицах: левая
панель~--- ошибка якобиана $\|B_k - J(x_k)\|_F/\|J(x_k)\|_F$, правая~---
относительная невязка $\|F(x_k)\|_2/\|F(x_0)\|_2$.
...
Классический Бройден ($p_{\max}=0$) стабилизируется на ненулевом
плато без~достижения сходимости (см.\ ниже); расширение
окна сдвигает плато ниже, причём прирост насыщается с~ростом~$p_{\max}$.
...
порога $\|F\|_2\le 10^{-10}$ ни на одном из~$20$ стартов и
стабилизируется на ненулевом относительном уровне~--- ограниченность
$\|E_k\|_F$ по теореме~\ref{thm:bd}, формально гарантируя локальную
теорию, эмпирически не транслируется в~сходимость по~$F$ на~этой задаче.
