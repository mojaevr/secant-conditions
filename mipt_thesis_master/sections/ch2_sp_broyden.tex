...
Левая панель~(а)~--- эмпирическая верификация ограниченности~$\|E_k\|_F$
из~\eqref{eq:bd_squared}: для всех $p_{\max}$ относительная ошибка
...
\caption{Discrete BVP, $n=100$, $\|F\|_2\le 10^{-10}$,
...
ограниченной~--- эмпирическая верификация bounded
deterioration~\eqref{eq:bd_squared}.
...
$p_{\max}\ge 5$ устойчиво сходится. Расширение окна
...
backtracking-Армихо ($c_1=10^{-4}$); критерий $\|F\|_2 \le 10^{-10}$.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig_highdim_conv.pdf}
\caption{Broyden Banded, $\|F\|_2\le 10^{-10}$, backtracking-Армихо
$c_1=10^{-4}$. Медианная траектория $\|F(x_k)\|_2$~+~IQR по случайным
стартам $x_0=-0{,}1\cdot\mathbf 1+0{,}05\,u_i$. Слева~--- $n=10^4$,
$8$~стартов; плотный Projected Broyden-SM ($p\le 5$, чёрный пунктир,
$3$~старта)~--- baseline. Справа~--- $n=10^5$, $4$~старта; плотная
форма требует $\approx 80$~ГБ оперативной памяти и неприменима.}
...
сходимость L-PB при~$m\ge p_{\max}+1$. Уменьшение окна до~$m=5$
($<p_{\max}+1=6$) вызывает стагнацию на~уровне $\sim\!10^{-9}$~---
прогнозируемый эффект потери ортогональности $v_k^\top s_j = 0$
для~уже~вытолкнутых $s_j$, входящих в~текущий $S_p^{(k)}$.
