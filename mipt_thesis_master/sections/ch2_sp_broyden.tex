...
\caption{Broyden Banded, $\|F\|_2\le 10^{-10}$, backtracking-Армихо
($c_1=10^{-4}$, $\psi$-форма). Медианная траектория $\|F(x_k)\|_2$~+~IQR
по случайным стартам $x_0=-0{,}1\cdot\mathbf 1+0{,}05\,u_i$.
Слева~--- $n=10^4$, $16$~стартов; плотный Projected Broyden-SM
($p\le 5$, чёрный пунктир, $6$~стартов)~--- baseline. Справа~--- $n=10^5$,
$8$~стартов; плотная форма требует $\approx 80$~ГБ оперативной памяти и
неприменима.}
\label{fig:highdim_conv}
\end{figure}

При $n=10^4$ плотный Projected Broyden-SM сходится за~$\sim\!50$ итераций;
L-PB при~$m=20$~--- за~$\sim\!75$, при~$m=10$~--- за~$\sim\!85$. Деградация
по~$m$ согласуется с~замечанием~\ref{rem:lpb_truncation}: L-PB и~плотная
форма \emph{не}~эквивалентны при~$k>m$, поскольку рекурсия~\eqref{eq:lsp_recursion}
неявно полагает $B_{k-m}^{\mathrm{LPB}}=I$, тогда как $B_{k-m}^{\mathrm{dense}}$
накопил историю обновлений (это типовая ситуация limited-memory методов,
ср.~L-BFGS~\ref{rem:lpb_vs_lbfgs}). Утверждение~\ref{prop:lpb_secant_retention}
гарантирует совпадение лишь действия на~$\mathrm{col}\,S_p^{(k)}$~---
последние $p_{\max}+1$ секущих условий~--- что и~обеспечивает
сходимость L-PB при~$m\ge p_{\max}+1$. При недостаточной ёмкости окна
($m\in\{3,5\} < p_{\max}+1=6$) обе соответствующие траектории
выходят на~плато на~уровне $\sim\!10^{-9}$ и~не~достигают $\mathtt{tol}=10^{-10}$
за~$300$~итераций~--- прогнозируемый эффект потери ортогональности
$v_k^\top s_j = 0$ для~уже~вытолкнутых $s_j$, входящих в~текущий $S_p^{(k)}$.
Кривая $m=5$ медленнее $m=3$ во~всей пре-асимптотической фазе:
при~$m=5$ формула $v_k = S_p G_p^{-1} e_1$ строится из~частично-устаревших
столбцов (4~предыдущих секущих хранятся, но~$v_k$ предполагает их
ортогональность к~уже~вытолкнутым), что создаёт более согласованную,
но~неверную структуру; при~$m=3$ обновление эффективно близко
к~ранг-1~Бройдену (используется $p_{\text{eff}}=2$~из-за~короткого~буфера)
и~работает хуже асимптотически, но~устойчивее в~средней фазе.
При $n=10^5$ L-PB устойчиво сходится при~$m\in\{10, 20\}$ за~$\sim\!85$
итераций; число итераций до~сходимости \emph{не растёт} с~$n$, что
демонстрирует применимость метода в~режиме, где плотная форма исчерпывает
память ($\approx 80$~ГБ при~$n=10^5$).
