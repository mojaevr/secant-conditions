...
\begin{proof}
Индукция по~$k$. При $k=0$ окно пусто, $B_0=I$, утверждение вакуумно.
Шаг: достаточно показать формулировку для~$B_{k+1}$, то~есть
$B_{k+1} s_j = y_j$ для~$j\in\{\max(0,k-p_{\max}),\ldots,k\}$;
для~этого используем гипотезу индукции лишь в~ослабленной форме~---
$B_k s_j = y_j$ для $j\in\{k-p_{\max},\ldots,k-1\}$ ($p_{\max}$~значений);
индекс $k-p_{\max}-1$ из~формулировки для~$B_k$ выталкивается из~окна
$S_p^{(k+1)}$ и~в~переходе не~участвует. Из~ранг-1
формы~\eqref{eq:rank1_specialization}
$B_{k+1} = B_k + (y_k - B_k s_k)v_k^\top$.
\begin{enumerate}
\item[(i)] $v_k^\top s_k = 1$ (из~$S_p^{(k),\top} v_k = G_p^{(k)} G_p^{(k),-1} e_1 = e_1$),
поэтому $B_{k+1} s_k = B_k s_k + (y_k - B_k s_k)\cdot 1 = y_k$.
\item[(ii)] Для $j\in\{k-p_{\max},\ldots,k-1\}$: вектор $s_j$ входит
в~$S_p^{(k)}$ как столбец с~номером $i = k-j+1 \in \{2,\ldots,p_{\max}+1\}$;
из~$S_p^{(k),\top} v_k = e_1$ получаем $s_j^\top v_k = (e_1)_i = 0$ (так как $i\ne 1$),
откуда $B_{k+1} s_j = B_k s_j = y_j$ по~индукции.
\end{enumerate}
Условие $m\ge p_{\max}+1$ обеспечивает, что все секущие из~$S_p^{(k)}$
доступны в~окне в~момент~$k$ (необходимо для~формирования $v_k$).
\end{proof}
...
$B_k^{\mathrm{LPB}}$ воспроизводит последние $\min(p_{\max}+1,m)$
секущих условий плотной формы, но~не~весь оператор $B_k^{\mathrm{dense}}$.
