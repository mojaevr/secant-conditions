...
$p+1$ шагов как наблюдаемая характеристика траектории. Локально, в~режиме
Q-сверхлинейной сходимости, шаги $s_j$ остаются линейно независимыми, и
$\kappa_p$ конечно; конкретное значение зависит от задачи и истории.
...
и~оценка bounded deterioration~\eqref{eq:bd_squared}
удерживается с~той~же константой $C_{\mathrm{PB}}$.
\end{proposition}

\begin{proof}
Индукция по~$k$. При $k=0$ окно пусто, $B_0=I$, утверждение вакуумно.
Шаг: пусть $B_k s_j = y_j$ для $j\in\{k-p_{\max},\ldots,k-1\}$
(индексы $<0$ опускаются). Из~ранг-1 формы~\eqref{eq:rank1_specialization}
$B_{k+1} = B_k + (y_k - B_k s_k)v_k^\top$.
\begin{enumerate}
