...
последний член по лемме~\ref{lem:fd} ограничен
...
из строки~6 алгоритма~\ref{alg:afd-inner} срабатывает. Если
\begin{equation}
...
&\le \frac{\Norm{g^{(0)}_k - B_k^{(0)} d^{(0)}_k}}{\sigma_{\min}(\widehat S_k)\,\Norm{d^{(0)}_k}} \\
&\le \frac{1}{\sigma_0}\,\frac{\Norm{g^{(0)}_k - J_k d^{(0)}_k} + \Norm{(J_k - B_k^{(0)})d^{(0)}_k}}{\Norm{d^{(0)}_k}} \\
&\le \frac{M_0 + L_1\tau\Norm{d^{(0)}_k}/2}{\sigma_0}.
\end{align*}
(Здесь использованы лемма~\ref{lem:fd} для FD-оценки и
...
AFD~--- $1 + 1 = 2$ вызова $F$. По сравнению с одним вызовом
...
\[
\#F_{\mathrm{\mathrm{AFD}}}(\varepsilon)
\;=\;
...
оптимальный режим $\mathcal O(T^{-3/2})$ для VIJI-Restarted~---
практически применимый вариант алгоритма с гарантированной
локально-оптимальной скоростью для произвольной $\mu$-сильно монотонной
$L_1$-гладкой VI без аналитического якобиана.
