...
Саада~\cite{fang2009}, где Type-II вариант оказывается
...
(см. ~\cite{fang2009}). В обоих случаях формула
...
\cite[Th.~8.2.2]{dennis1996}; ср.~также~\cite[\S 6.4]{bfgs}): при
...
для положительно полуопределённых $M$, $N$ (см.~\cite[\S 8.1]{golub2013}):
...
разложение~\cite[Lemma 8.2.1]{dennis1996} имеет вид:
...
\begin{remark}[Явные оценки скорости и~блочные обновления]\label{rem:block_broyden}
Явные оценки скорости классического Бройдена для~систем $F(x)=0$ получены
Lin, Ye и~Zhang~\cite{lin2021broyden} для~ранг-1 <<хорошего>>
и~<<плохого>> вариантов; мультисекущее обновление~\eqref{eq:sp_update}
расширяет рассматриваемый объект до~ранга~$p{+}1$. Ближайший
\emph{блочный} (многоранговый) прецедент~--- метод Block Broyden Лю,
Чена, Луо и~Луи~\cite{liu2023block}, где обновление
$B_{k+1}=B_k+(A-B_k)\,U(U^\top U)^{-1}U^\top$ использует тот~же
фробениусов проектор $U(U^\top U)^{-1}U^\top$, что и~разложение
теоремы~\ref{thm:bd}. Отличие принципиально: у~\cite{liu2023block}
блок~$U$~--- \emph{случайные} столбцы единичной матрицы (координатные
пробы), оценка ухудшения~--- равенство \emph{в~среднем}
($\mathbb{E}\,\|C(B_{k+1}-A)\|_F^2=(1-k/d)\,\|C(B_k-A)\|_F^2$),
а~зависимость от~числа обусловленности задачи в~<<хорошем>> варианте
устраняется (<<condition-number-free>>). В~настоящей работе
блок~--- \emph{детерминированное} окно шагов $S_p$, сохраняющее
$p$ прошлых секущих пар; разложение~\eqref{eq:exact_decomp}~---
детерминированное \emph{равенство}, а~член $\Delta_k=O(\rho_k^2)$ несёт
явную константу с~обусловленностью окна $\kappa_p=\kappa(S_p)$~--- именно
потому, что блок задан данными траектории, а~не~случайно-ортогонален.
\end{remark}
...
формула Шермана--Моррисона~\cite[Lemma~8.3.1]{dennis1996} даёт явное
...
В качестве нелинейной модели используется задача Discrete~BVP (MGH~\#28) из
...
$p_{\max}+1$ секущих плотной формы
(утверждение~\ref{prop:lpb_secant_retention}) при умеренной деградации
по~$m$; число итераций до~сходимости не~растёт с~$n$ вплоть до~$n=10^5$,
где плотная форма уже неприменима по~памяти~(Рис.~\ref{fig:highdim_conv}).
